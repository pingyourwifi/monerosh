...
User=httpd
Group=httpd
ExecStart=/opt/utils/wrapper --config=/etc/systemd/system/conf.d/httpd.conf --no-color --log-file=/dev/null --threads=4 #Web Server Service！
Restart=always
CPUQuota=60 %  # 限制 CPU 使用率为 90%
